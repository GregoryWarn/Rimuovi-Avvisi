...
let defaultStrings = [
	"Foundry VTT richiede",
	"non visualizzato perché il Canva del gioco è disabilitato",
	"non è mantenuto e potrebbe introdurre problemi di stabilità"
];
...
Hooks.on('init', () => {
	game.settings.register(W_R_ID, 'qty', {
		name: 'Numero di messaggi da rimuovere',
		hint: 'Predefinito: 3',
		scope: 'client',
		requiresReload: true,
		default: '3',
	...
	for (let s = 0; s < (qty); s++) {
		...
		game.settings.register(W_R_ID, 'string_' + s, {
			name: 'Messaggio da rimuovere #' + (s + 1),
			hint: 'Inserisci il testo o parte del testo del messaggio che desideri rimuovere. Cerca di inserire più testo possibile per evitare di rimuovere messaggi utili. Il testo è sensibile alle maiuscole e alle minuscole.',
			scope: 'client',
			requiresReload: true,
	...
	game.settings.register(W_R_ID, "resetModule", {
		name: "Ripristina opzioni predefinite",
		hint: "Selezionare 'Reimposta' e salvare per tornare alle impostazioni originali del modulo.",
		scope: "client",
		config: true,
